...
static void my_igmp_group_thing(const struct sbrec_igmp_group *sb_igmp, const struct ovn_datapaths *ls_datapaths, struct ovsdb_idl_index *sbrec_mcast_group_by_name_dp, struct hmap *igmp_groups, const struct hmap *ls_ports);
...
//        struct lflow_ref *lflow_ref)
    ...
    HMAP_FOR_EACH_SAFE(mcast, hmap_node, &mcast_groups){
        ...
        const struct sbrec_multicast_group *mcgroup =
            ...
            ovn_multicast_update_sbrec(mcast, mcgroup);
            ovn_multicast_destroy(&mcast_groups, mcast);
    }
    return true;
}
