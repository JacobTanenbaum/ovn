...
struct advertized_routes_for_table_id_entry{
    struct hmap_node node;
                   ...
                   struct route_exchange_ctx_out *r_ctx_out)
{
    ...
    struct hmap advertized_routes = HMAP_INITIALIZER(&advertized_routes);
    ...
    HMAP_FOR_EACH (ad, node, r_ctx_in->announce_routes) {
        ...
        struct advertized_routes_for_table_id_entry *entry;
        uint32_t hash = maintained_route_table_hash(table_id);
        HMAP_FOR_EACH_WITH_HASH (entry, node, hash, &advertized_routes) {
            if (entry && entry->table_id == table_id && !hmap_is_empty(&entry->routes)) {
                VLOG_ERR("KEYWORD: CHANGE THIS hmap_count(&ad->routes) = %"PRIu64" hmap_count(&entry->routes) = %"PRIu64"", hmap_count(&ad->routes), hmap_count(&entry->routes));
            }
        }
        if (entry == NULL) {
            entry = xmalloc(sizeof *entry);
            *entry = (struct advertized_routes_for_table_id_entry) {
                .table_id = table_id,
                .routes = HMAP_INITIALIZER(&entry->routes),
            };
            hmap_insert(&advertized_routes, &entry->node, hash);
        }
        ...
        HMAP_FOR_EACH(are, node, &ad->routes) {
            ...
            *copy = (struct advertise_route_entry) {
            ...
            hmap_insert(&entry->routes, &copy->node, hmap_node_hash(&are->node));
        }
    ...
    struct advertized_routes_for_table_id_entry *arte;
    HMAP_FOR_EACH_POP (arte, node, &advertized_routes) {
    ...
    hmap_destroy(&advertized_routes);
}
